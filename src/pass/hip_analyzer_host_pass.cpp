...
llvm::SmallVector<llvm::Function*, 8>
GlobalMemoryQueueHostPass::createInstrumentationStubs(
    llvm::Function& original_stub) {
...
llvm::Function* GlobalMemoryQueueHostPass::replaceStubCall(
    llvm::Function& stub,
    llvm::ArrayRef<llvm::Function*> instrumentation_stubs) const {
    auto& mod = *stub.getParent();
    auto& context = mod.getContext();
...
llvm::SmallVector<llvm::Function*, 8>
CUMemoryTraceHostPass::createInstrumentationStubs(
    llvm::Function& original_stub) {
...
llvm::Function* CUMemoryTraceHostPass::replaceStubCall(
    llvm::Function& stub,
    llvm::ArrayRef<llvm::Function*> instrumentation_stubs) const {
    auto& mod = *stub.getParent();
    auto& context = mod.getContext();
    ...
    queue_info = builder.CreateCall(instr_handlers.newCUMemQueueInfo,
                                    {builder.getInt64(32)});

    events_buffer = builder.CreateCall(instr_handlers.hipCUMemQueueInfoToDevice,
                                       {queue_info});
    ...
    // Store counters (runtime)
    builder.CreateCall(instr_handlers.hipCUMemQueueInfoRecord,
                       {queue_info, events_buffer});

    builder.CreateCall(instr_handlers.freeHipCUMemoryQueueInfo, {queue_info});
...
llvm::SmallVector<llvm::Function*, 8>
ChunkAllocatorHostPass::createInstrumentationStubs(
    llvm::Function& original_stub) {

    // Kind of a hack but technically the same kernel signature
    auto dummy_trace = TraceType::create("trace-wavestate-chunkallocator");
    return {
        addTracingDeviceStub(original_stub, *dummy_trace),
    };
}

llvm::Function* ChunkAllocatorHostPass::replaceStubCall(
    llvm::Function& stub,
    llvm::ArrayRef<llvm::Function*> instrumentation_stubs) const {
    auto& mod = *stub.getParent();
    auto& context = mod.getContext();
